...
describe('plugin', () => {
	...
	const dummyLoaderPath = require.resolve('./fixtures/__shared__/dummy-loader.js');
	...
	function activeChunk(file) {
		superagent
			.get(lazyPlugin.server.createActivationUrl(getPath('simple', file)).replace('{host}', 'localhost'))
	...
	it('should generate magic code for async module', async () => {
		...
		expect(lazyStats.compilation.assets['bundle.js'].source()).toEqual(expect.stringContaining(`@activationUrl ${lazyPlugin.server.createActivationUrl(getPath('simple', 'index.js'))}`));
	});
	...
	it('should trigger recompile for the real moudle when module is actived', done => {
		...
		waitForCompile(compiler, () => {
			...
			.then(stats => {
				expect(dummyLoader.loader).not.toHaveBeenCalled();
				expect(stats.compilation.assets['a.chunk.js'].source()).toEqual(expect.stringContaining(`@activationUrl ${lazyPlugin.server.createActivationUrl(getPath('simple', 'a.js'))}`));
				activeChunk('a.js');
			})
			.then(stats => {
				expect(dummyLoader.loader).toHaveBeenCalled();
				expect(stats.compilation.assets['a.chunk.js'].source()).toEqual(expect.not.stringContaining(`@activationUrl ${lazyPlugin.server.createActivationUrl(getPath('simple', 'a.js'))}`));
			})
			.then(done);
	});
});
